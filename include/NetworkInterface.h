...
protected:
  ...
  */
  ...
  std::map<std::pair<AlertEntity, std::string>, AlertableEntity *>
      external_alerts;
...
public:
  ...
  */
  ...
  inline int exec_sql_query([[maybe_unused]] lua_State *vm,
                            [[maybe_unused]] char *sql,
                            [[maybe_unused]] bool limit_rows,
                            [[maybe_unused]] bool wait_for_db_created = true) {
#ifdef HAVE_MYSQL
    // FIXME: this is bad for performance
    if (dynamic_cast<MySQLDB *>(db) != NULL)
      return ((MySQLDB *)db)
          ->exec_sql_query(vm, sql, limit_rows, wait_for_db_created);
#endif
    return (-1);
  };
  ...
  /* unlockExternalAlertable must be called after use whenever a non-null
   * reference is returned */
  AlertableEntity *lockExternalAlertable(AlertEntity entity,
