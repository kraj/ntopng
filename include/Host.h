...
public:
  ...
  */
  ...
  inline const u_int8_t *get_mac() const {
  ...
  virtual void
  custom_periodic_stats_update([[maybe_unused]] const struct timeval *tv) {
    ;
  }
  ...
  inline void decNumAlertedFlows([[maybe_unused]] bool as_client) {
    active_alerted_flows--;
  }
  ...
  virtual void luaDNS([[maybe_unused]] lua_State *vm,
  ...
  virtual NetworkStats *getNetworkStats([[maybe_unused]] int16_t networkId) {
    return (NULL);
  };
  inline Country *getCountryStats() { return country; };

  bool match([[maybe_unused]] const AddressTree *const tree) const {
    return ip.match(tree);
  };
  ...
  inline void reloadHideFromTop() {
    hidden_from_top = iface->isHiddenFromTop(this);
  }
  inline void reloadDhcpHost() {
    is_dhcp_host = iface->isInDhcpRange(get_ip());
  }
