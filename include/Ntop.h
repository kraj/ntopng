...
public:
   ...
   */
  inline void resolveHostName(char *numeric_ip, char *symbolic,
                              u_int symbolic_len) {
  ...
  inline char *get_plugins_dir() {
    return (plugins0_active ? plugins0_dir : plugins1_dir);
  };
  inline char *get_shadow_plugins_dir() {
    return (plugins0_active ? plugins1_dir : plugins0_dir);
  };
  ...
  bool recipients_enqueue(RecipientNotificationPriority prio,
  ...
  bool recipient_dequeue(u_int16_t recipient_id,
                         RecipientNotificationPriority prio,
                         AlertFifoItem *notification);
  ...
  inline u_int importClickHouseDumps(bool silence_warnings) {
    return (clickhouseImport.importDumps(silence_warnings));
  }
#endif
