...
AddressResolution::~AddressResolution() {
  ...
  ntop->getTrace()->traceEvent(
      TRACE_NORMAL, "Address resolution stats [%u resolved][%u failures]",
      num_resolved_addresses, num_resolved_fails);
}

/* ***************************************** */

void AddressResolution::resolveHostName(char *_numeric_ip, char *symbolic,
                                        u_int symbolic_len) {
  ...
  if (ntop->getRedis()->getAddress(numeric_ip, rsp, sizeof(rsp), false) < 0) {
    ...
    if (!isxdigit(numeric_ip[numeric_ip_len]) &&
      ...
      ntop->getRedis()->setResolvedAddress(numeric_ip,
                                           h ? h->h_name : (char *)"");
...
bool AddressResolution::resolveHost(char *host, char *rsp, u_int rsp_len,
                                    bool v4) {
