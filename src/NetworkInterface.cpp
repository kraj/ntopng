  ...
  if (ret == NULL) {
    ...
    *new_flow = true;
    ...
    try {
      ...
      ret = new (std::nothrow) Flow(
          this, vlan_id, observation_domain_id, l4_proto, srcMac, src_ip,
          src_port, dstMac, dst_ip, dst_port, icmp_info, first_seen, last_seen);
...
pre_get_flow:
  /* Updating Flow */
  flow = getFlow(srcMac, dstMac, vlan_id, 0 /* observationPointId */, 0, 0, 0,
                 l4_proto == IPPROTO_ICMP ? &icmp_info : NULL, &src_ip, &dst_ip,
                 src_port, dst_port, l4_proto, &src2dst_direction,
                 last_pkt_rcvd, last_pkt_rcvd, len_on_wire, &new_flow, true);
  ...
  if (companionQueue) {
    ...
    while (dequeueFlowFromCompanion(&dequeued)) {
      ...
      flow = getFlow(NULL /* srcMac */, NULL /* dstMac */, 0 /* vlan_id */,
                     0 /* observationPointId */, 0 /* deviceIP */,
                     0 /* inIndex */, 1 /* outIndex */, NULL /* ICMPinfo */,
                     &dequeued->src_ip, &dequeued->dst_ip, dequeued->src_port,
                     dequeued->dst_port, dequeued->l4_proto, &src2dst_direction,
                     0, 0, 0, &new_flow, true /* create_if_missing */);
...
bool NetworkInterface::initFlowDump(u_int8_t num_dump_interfaces) {
  ...
  if (db == NULL) {
    if (ntop->getPrefs()->do_dump_flows_on_mysql()) {
      ...
      if ((db == NULL) && ntop->getPrefs()->is_enterprise_m_edition())
...
AlertableEntity *NetworkInterface::lockExternalAlertable(
...
void NetworkInterface::unlockExternalAlertable(AlertableEntity *alertable) {
  if (alertable->getNumEngagedAlerts() == 0) {
    std::pair<AlertEntity, std::string> key(alertable->getEntityType(),
                                            alertable->getEntityValue());
