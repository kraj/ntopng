...
bool Recipients::dequeue(u_int16_t recipient_id,
                         RecipientNotificationPriority prio,
...
bool Recipients::enqueue(RecipientNotificationPriority prio,
                         const AlertFifoItem *const notification,
  ...
  /*
     Perform the actual enqueue for the given priority to all available
     recipients
   */
    ...
    if (recipient_queues[recipient_id] &&
        ((alert_entity == alert_entity_flow &&
          recipient_queues[recipient_id]
              ->isFlowRecipient()) /* The recipient must be a flow recipient */
         || (alert_entity == alert_entity_host &&
             recipient_queues[recipient_id]->isHostRecipient()))) {
      bool success =
          recipient_queues[recipient_id]->enqueue(prio, notification);
      res &= success;
    }
  }
  ...
  for (int recipient_id = 0; recipient_id < MAX_NUM_RECIPIENTS;
    ...
    if (recipient_queues[recipient_id]) {
      if (flow_recipients & (1 << recipient_id))
        recipient_queues[recipient_id]->setFlowRecipient(
            true /* This is a flow recipient */);
      else
        recipient_queues[recipient_id]->setFlowRecipient(
            false /* This is NOT a flow recipient */);

      // ntop->getTrace()->traceEvent(TRACE_WARNING, "Set flow recipient
      // [%u][%u]", recipient_id, flow_recipients & (1 << recipient_id));
    }
  }
  ...
  for (int recipient_id = 0; recipient_id < MAX_NUM_RECIPIENTS;
    ...
    if (recipient_queues[recipient_id]) {
      if (host_recipients & (1 << recipient_id))
        recipient_queues[recipient_id]->setHostRecipient(
            true /* This is a host recipient */);
      else
        recipient_queues[recipient_id]->setHostRecipient(
            false /* This is NOT a host recipient */);

      // ntop->getTrace()->traceEvent(TRACE_WARNING, "Set host recipient
      // [%u][%u]", recipient_id, host_recipients & (1 << recipient_id));
    }
  }
