...
void Flow::processDetectedProtocol() {
  ...
  case NDPI_PROTOCOL_NTP:
    if (srv_h) {
      srv_h->setNtpServer();
      if (cli_h)
        cli_h->incNTPContactCardinality(srv_h);
    }
    break;
...
void Flow::processIEC60870Packet(bool tx_direction,
                                 [[maybe_unused]] const u_char *ip_packet,
                                 [[maybe_unused]] u_int16_t ip_len,
...
void Flow::setProtocolDetectionCompleted() {
  ...
  stats.setDetectedProtocol(&ndpiDetectedProtocol);
  processDetectedProtocol();     /* Process detected protocol and doesn't need
                                    ndpiFlow not allocated for non-packet interfaces
                                  */
  processDetectedProtocolData(); /* Process detected protocol data and needs
                                    ndpiFlow only allocated for packet
                                    interfaces      */
...
bool Flow::equal(const IpAddress *_cli_ip, const IpAddress *_srv_ip,
                 ...
                 [[maybe_unused]] u_int16_t _observation_point_id,
...
bool Flow::enqueueAlertToRecipients(FlowAlert *alert) {
  ...
  /* TODO: read all the recipients responsible for flows, and enqueue only to
   * them */
  ...
  rv = ntop->recipients_enqueue(notification.alert_severity >= alert_level_error
                                    ? recipient_notification_priority_high
                                    : recipient_notification_priority_low,
                                &notification,
...
void Flow::updateTcpSeqNum([[maybe_unused]] const struct bpf_timeval *when,
...
void Flow::dissectDNS([[maybe_unused]] bool src2dst_direction, char *payload,
...
void Flow::dissectSSDP([[maybe_unused]] bool src2dst_direction, char *payload,
                       u_int16_t payload_len) {
  char url[512];
  u_int i = 0;
