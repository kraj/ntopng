...
TrafficStats::TrafficStats(const TrafficStats &ts) {
  numPkts = ts.numPkts;
  numBytes = ts.numBytes;
};
TrafficStats::TrafficStats(TrafficStats &&ts);
{
  numPkts = std::move(ts.numPkts);
  numBytes = std::move(ts.numBytes);
};
TrafficStats &TrafficStats::operator=(const TrafficStats &ts) {
  if (&this == ts) {
    return *this;
  }
  numPkts = ts.numPkts;
...
TrafficStats &TrafficStats::operator=(TrafficStats &&ts) {
  numPkts = std::move(ts.numPkts);
  numBytes = std::move(ts.numBytes);
}
TrafficStats::~TrafficStats() { numPkts.reset(), numBytes.reset(); }
