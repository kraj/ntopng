  ...
  if ((strncmp(request_info->uri, "/lua/", 5) == 0) ||
    ...
    if ((!whitelisted) && isCaptiveConnection(conn) &&
      ...
      if (strcmp(request_info->uri, "/metrics") == 0)
        ...
        snprintf(path, sizeof(path), "%s/scripts/%s", ntop->get_plugins_dir(),
                 request_info->uri + 9);
      else
...
HTTPserver::HTTPserver(const char *_docs_dir, const char *_scripts_dir) {
   ...
   * httpdocs directory for the plugins. Need to configure the rewrite for both
   * the plugins0 and plugins1 directories. See plugins_utils.getHttpdocsDir */
